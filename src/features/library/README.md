...
Items don't carry a tier of their own in the dataset, and this slice no
longer derives one. `ItemsTable` reads the planner's `list_item_tiers`
through `useItemTiers`, which walks the whole input chain — a local
derivation off recipe stamps read a byproduct as an item's origin and
disagreed with the planner about Water, Crude Oil and Nitrogen Gas.
